...
| Embedding + rerank credentials (Voyage) | Us | §7.3, §7.4 | **Provisioned and used** — `voyage-4-large` embeddings (R-058), cached to disk so a scored run reaches no network. `rerank-2` was measured and **not adopted**: it moved pack recall by zero cases on top of dense (R-062, Q4 answered) |
...
| **Mathpix API key** | Client | formula extraction (R-008, R-037, R-043) | Not provisioned, and **less urgent than it looked**: Chandra's LaTeX was already on disk and unused. 1 of the 5 unusable formulas is recovered safely; the other 4 need a human to pick among candidates, not a new vendor |
